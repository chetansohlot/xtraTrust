...
def index(request):
    ...
    employees_qs = Employees.objects.all().order_by('-created_at')
...
def employee_basic_info(request,employee_id=None):
    if not request.user.is_authenticated and request.user.is_active!=1:
        messages.error(request,'Please Login First')
        return redirect('login')
    
    clients_list = XtClientsBasicInfo.objects.filter(active='active')  # always fetch

    if employee_id:
        employee_info = get_object_or_404(Employees, employee_id=employee_id)
    else:
        employee_info = None  # For creating new employee

    return render(request, 'employee/create.html', {
        'employee_info': employee_info ,'clients_list': clients_list
    })
...
def save_or_update_employee(request, employee_id=None):
    ...
    if request.method != 'POST':
        ...
        if employee_id:
            ...
            clients_list = XtClientsBasicInfo.objects.filter(active='active')   
        return render(request, 'employee/create.html', {
            'employee_info': employee_instance,
            'clients_list': clients_list,
        })

    try:
        # Cleaned data from POST
        ...
        # Prepare employee_data for repopulation in case of error
        employee_data = {
            'first_name': first_name,
            'last_name': last_name,
            'dob': dob,
        ...
        try:
            ...
            messages.error(request, "Invalid date format in DOB field.")
            return render(request, 'employee/create.html', {'employee_info': employee_data})
        ...
        if dob_date:
            ...
            if age < 18:
                messages.error(request, "Employee must be at least 18 years old.")
                return render(request, 'employee/create.html', {'employee': employee_data})
        ...
        # duplicate = Employees.objects.filter(aadhaar_card=aadhaar_card)
        # if employee_id:
        #     duplicate = duplicate.exclude(employee_id=employee_id)
        ...
        employee = Employees.objects.filter(employee_id=employee_id).first() if employee_id else Employees()
        created = not bool(employee_id)

        employee.first_name = first_name
        employee.last_name = last_name
        employee.date_of_birth = dob_date
        employee.gender = gender
        employee.pan_card = pan_no
        ...
        employee.save()
        messages.success(request, f"Employee {'created' if created else 'updated'} successfully.")
        return redirect('employee-management-update-address', employee_id=employee.employee_id)
...
def employee_address_info(request, employee_id=None):
    if not request.user.is_authenticated and request.user.is_active!=1:
        messages.error(request,'Please Login First')
        return redirect('login')

    employee_info = get_object_or_404(Employees, employee_id=employee_id) if employee_id else None

    # Load address info
    permanent_address = Address.objects.filter(employee_id=employee_id, type="permanent").first() if employee_id else None
    correspondence_address = Address.objects.filter(employee_id=employee_id, type="correspondence").first() if employee_id else None

    states = State.objects.all()
    permanent_cities = City.objects.filter(state_id=permanent_address.state) if permanent_address else []
    correspondence_cities = City.objects.filter(state_id=correspondence_address.state) if correspondence_address else []
...
def save_or_update_address(request, employee_id):
    ...
    if request.method != "POST":
        return redirect('employee-management-update-address', employee_id=employee_id)

    # Validate only permanent address fields first
    perm_addr = request.POST.get("permanent_address", "").strip()
    perm_state = request.POST.get("permanent_state", "").strip()
    perm_city = request.POST.get("permanent_city", "").strip()
    perm_pincode = request.POST.get("permanent_pincode", "").strip()

    errors = False

    if not perm_addr:
        messages.error(request, "Permanent Address is required.")
        errors = True
    if not perm_state:
        messages.error(request, "Permanent State is required.")
        errors = True
    if not perm_city:
        messages.error(request, "Permanent City is required.")
        errors = True
    if not perm_pincode or not perm_pincode.isdigit() or len(perm_pincode) != 6:
        messages.error(request, "Invalid Permanent Pincode.")
        errors = True

    # if errors:
    #     return redirect(request.META.get('HTTP_REFERER', '/'))

    # Now save or update both addresses (permanent + correspondence)
    for addr_type in ["permanent", "correspondence"]:
        addr = request.POST.get(f"{addr_type}_address", "").strip()
        state = request.POST.get(f"{addr_type}_state", "").strip()
        city = request.POST.get(f"{addr_type}_city", "").strip()
        pincode = request.POST.get(f"{addr_type}_pincode", "").strip()

        try:
           address_obj = Address.objects.get(employee_id=employee.employee_id, type=addr_type)
        except Address.DoesNotExist:
           address_obj = Address(employee_id=employee.employee_id, type=addr_type, created_at=now())
    ...
    messages.success(request, "Employee addresses updated successfully.")
    return redirect('employee-management-employment-info', employee_id=employee.employee_id)


def employee_family_details(request, employee_id=None):
    if not request.user.is_authenticated and request.user.is_active!=1:
        messages.error(request,'Please Login First')
        return redirect('login')
...
def employee_employment_info(request, employee_id=None):
    if not request.user.is_authenticated and request.user.is_active != 1:
        messages.error(request, "Please login first.")
        return redirect('login')
    ...
    employment = EmploymentInfo.objects.filter(employee_id=employee_id).first()
    company_emp = settings.COMPANY_EMP
    employment_code = f"{company_emp}-{10000 + int(employee_id)}"

    # Prepare context with existing employment info or empty defaults
    context = {
    ...
    return render(request, 'employee/create-employee-info.html', context)
...
def employee_references_info(request, employee_id):
...
def save_or_update_references(request, employee_id):
    if not request.user.is_authenticated or request.user.is_active != 1:
        messages.error(request, "Please login first.")
        return redirect('login')
    ...
    if request.method != "POST":
        return redirect('employee-management-update-references', employee_id=employee_id)
...
def employee_delete(request,employee_id):
    try:
        employees = get_object_or_404(Employees,employee_id=employee_id)

        # Status change krna 
        employees.active = not employees.active
        employees.save()

        employees.save()

        return JsonResponse({'success': True, 'new_status': employees.active})
    except Exception as e:
